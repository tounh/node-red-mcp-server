 ...
 * @param {Object} customHeaders - Custom HTTP headers (optional)
 * @returns {Promise<any>} Result of the API call
 */
export async function callNodeRed(method, path, data = null, config, responseType = 'json', customHeaders = {}) {
  ...
  if (config.nodeRedUsername && config.nodeRedPassword) {
    ...
    headers = { ...headers, 'Authorization': 'Bearer ' + config.nodeRedToken };
  }
  ...
  try {
    ...
    if (error.response?.status === 401) {
      if (authManager && config.nodeRedUsername) {
        ...
        try {
          const authHeaders = await authManager.getAuthHeaders();
          const retryHeaders = { ...customHeaders, ...authHeaders };
          const retryAxiosConfig = { method, url, headers: retryHeaders, data, timeout: 30000 };
          if (responseType === 'text') {
